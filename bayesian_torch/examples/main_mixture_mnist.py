...
import bayesian_torch.models.mixture.simple_cnn as simple_cnn
len_trainset = 60000
len_testset = 10000
...
def train(args, model, device, train_loader, optimizer, epoch, tb_writer=None):
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        kl_weight = 2**(len(train_loader) -
                        batch_idx) / (2**len(train_loader) - batch_idx)

        scaled_kl = kl * kl_weight
        loss = nll_loss + scaled_kl
        loss.backward()
        optimizer.step()
    ...
    with torch.no_grad():
        ...
        for data, target in test_loader:
            ...
            for mc_run in range(args.num_monte_carlo):
                ...
                #get probsalities from log-prob
